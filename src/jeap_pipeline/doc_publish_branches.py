...
The patterns are **GitHub's branch filters**, the syntax a team has already written under
`on: push: branches:`, so the same list means the same thing in both places.
"""
    ...
    for pattern in patterns:
        if not isinstance(pattern, str) or not pattern.strip():
            raise DocumentationConfigError(
                ...
                f"pattern. Every entry is a branch name or a GitHub branch filter, written as a "
                f"string.")
...
def branch_matches(branch: str, pattern: str) -> bool:
    """
    Whether a branch name matches one GitHub branch filter.

    The syntax is the one of `on: push: branches:`, so a list written for the one behaves like the
    other: `*` matches anything but a `/`, `**` matches across `/` as well, `?` matches a single
    character that is not a `/`, and everything else matches itself.

    Args:
        branch (str): The branch name, without `refs/heads/`.
        pattern (str): The filter, for instance `master`, `release/*` or `feature/**`.

    Returns:
        bool: Whether the filter matches the whole branch name.
    """
...
def _expression_of(pattern: str) -> str:
    """
    Translate a branch filter into an anchored regular expression.

    Deliberately not `fnmatch`, whose `*` crosses a `/`: `release/*` would then match
    `release/1.2/hotfix`, which is not what the same list means as a GitHub filter.
    """
    expression = []
    index = 0
