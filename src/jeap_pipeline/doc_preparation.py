...
def requires_asciidoc_conversion(configuration: dict) -> bool:
    ...
    _, conversions = _normalize(configuration)
...
def prepare_documentation_config(configuration: dict, output_directory: str,
    ...
    """
    prepared, conversions = _normalize(configuration)
    ...
    for entry in configuration["docs"]:
    ...
    for index, entry in conversions:
        ...
        prepared["docs"][index]["path"] = set_root.relative_to(root).as_posix()
    return prepared
...
def _normalize(configuration):
    ...
    conversions = []
    if isinstance(prepared, dict) and isinstance(prepared.get("docs"), list):
        for index, entry in enumerate(prepared["docs"]):
            ...
            if not isinstance(location, str) or not re.fullmatch(r"[a-z0-9]+(?:-[a-z0-9]+)*", location):
                raise DocumentationConfigError(f"docs[{index}]: AsciiDoc requires a chapter slug in 'location'.")
            ...
            if (not isinstance(entry_file, str) or not entry_file or "\\" in entry_file
                ...
                raise DocumentationConfigError(f"docs[{index}]: 'entry' must be a relative file inside 'path'.")
    ...
    for index, document in enumerate(sets):
        ...
        if identity in seen:
            raise DocumentationConfigError(
                f"docs[{seen[identity]}] and docs[{index}] publish the same documentation set after "
